...
@pytest.mark.asyncio
async def test_quiz_evaluator_returns_query_trace_when_evidence_search_runs(tmp_path) -> None:
    from src.agent.tools.quiz_evaluator import QuizEvaluatorArgs, QuizEvaluatorTool
    from src.agent.types import LlmResponse, ToolContext
    ...
    llm.send_request = AsyncMock(
        side_effect=[
            ...
            LlmResponse(content="根据课程资料，TCP 需要三次握手 [1]。"),
        ]
    )
    hybrid_search = MagicMock()
    hybrid_search.search = MagicMock(
    ...
    assert trace["metadata"]["source"] == "quiz_evaluator"
    assert trace["metadata"]["parent_agent_trace_id"] == "agent-trace-1"


@pytest.mark.asyncio
async def test_quiz_evaluator_direct_mode_does_not_fabricate_query_trace() -> None:
    from src.agent.tools.quiz_evaluator import QuizEvaluatorArgs, QuizEvaluatorTool
    from src.agent.types import LlmResponse, ToolContext
    ...
    llm.send_request = AsyncMock(
        return_value=LlmResponse(
            content='{"verdict":"incorrect","score":40,"explanation":"答案不完整","key_concepts":[]}'
        )
    )

    tool = QuizEvaluatorTool(llm_service=llm, hybrid_search=None, trace_enabled=True)
    result = await tool.execute(
        ToolContext(user_id="u1", conversation_id="c1"),
        QuizEvaluatorArgs(
            question="TCP 为什么需要三次握手？",
    ...
    assert result.metadata["query_trace_ids"] == []
    assert result.metadata["query_trace_id"] == ""
