...
## Phase S: Agent 模块深度优化

### 目标

对 `src/agent/` 全模块进行面试级深度审计与修复，覆盖关键 Bug、架构缺陷、并发安全、安全防护和代码质量。
...
| ID | 问题 | 文件 | 修复方案 |
|----|------|------|----------|
| S1 | 错题次数硬编码 `{1}` | `memory/enhancer.py:186` | 改用 Counter 统计同 topic 错误次数 |
| S2 | `asyncio.create_task` fire-and-forget | `agent.py:209`, `enhancer.py:450` | Task 集合跟踪 + `add_done_callback` 异常日志 |
| S3 | 工具中同步阻塞 async 事件循环 | `quiz_generator.py`, `review_summary.py`, `document_ingest.py` | `await asyncio.to_thread(...)` |
| S4 | verdict 大小写不一致 | `quiz_evaluator.py:110` | `.lower().strip()` 标准化 |
| S5 | FeedbackStore 使用同步 sqlite3 | `memory/feedback_store.py` | 全面迁移到 aiosqlite |

#### P1 — 架构缺陷（8 项）

| ID | 问题 | 修复方案 |
|----|------|----------|
| S6 | `prompt_builder.build()` 使用 `str.format()` 易注入 | 改用 `string.Template.safe_substitute` |
| S7 | agent.py 后台任务无跟踪 | Task 集合 + `flush()` graceful shutdown |
| S8 | `datetime.now()` 无时区 | 改用 `datetime.now(timezone.utc)` |
| S9 | `model_validate_json` 无异常保护 | 所有 memory 模块加 try/except + 默认值降级 |
| S10 | `FileConversationStore.update()` 并发不安全 | 加 `asyncio.Lock` per user |
| S11 | `document_ingest` 未检查符号链接 | `path.is_symlink()` 检查 |
| S12 | `knowledge_query._ensure_initialized()` 非线程安全 | `asyncio.Lock` 保护 |
| S13 | `update_profile` 允许任意字段覆盖 | 白名单限制可修改字段 |

#### P2 — 性能优化（2 项）

| ID | 问题 | 修复方案 |
|----|------|----------|
| S14 | `apply_decay` 逐条 UPDATE + 逐次 commit | 批量收集后单次 `executemany` + 单次 commit |
| S15-S16 | `session_memory.get_topic_history` / `error_memory.get_errors` topic 过滤在 Python 层 | SQL LIKE 下推 |

#### P3 — 代码质量（4 项）

| ID | 问题 | 修复方案 |
|----|------|----------|
| S17 | `observability.py` 文件写入无缓冲 | 异常安全 + 使用 `logging.FileHandler` 降级 |
| S18 | `enhancer.py._save_tool_chains` 内部重复 `import asyncio` | 移至模块级 |
| S19 | `review_schedule.py` `datetime.now()` 无时区 | 统一 UTC |
| S20 | LLM JSON 响应解析缺乏统一防护 | 提取 `safe_parse_json()` 工具函数 |

### 受影响文件

```
src/agent/agent.py                    — S2, S7
src/agent/types.py                    — S8
src/agent/prompt_builder.py           — S6
src/agent/conversation.py             — S10
src/agent/memory/enhancer.py          — S1, S2, S18
src/agent/memory/student_profile.py   — S9, S13
src/agent/memory/knowledge_map.py     — S9, S14, S8
src/agent/memory/error_memory.py      — S9, S15
src/agent/memory/session_memory.py    — S9, S16
src/agent/memory/feedback_store.py    — S5
src/agent/tools/quiz_evaluator.py     — S4, S20
src/agent/tools/quiz_generator.py     — S3, S20
src/agent/tools/review_summary.py     — S3
src/agent/tools/document_ingest.py    — S3, S11
src/agent/tools/knowledge_query.py    — S12
src/agent/hooks/review_schedule.py    — S19
src/agent/hooks/observability.py      — S17
src/agent/utils/sanitizer.py          — (已有)
src/agent/utils/json_helpers.py       — S20 (新增)
```
...
Agent.chat()
  └─ yield DONE event
  └─ task = asyncio.create_task(_post_message_tasks)
  └─ self._bg_tasks.add(task)
  └─ task.add_done_callback(_on_task_done)
       ├─ self._bg_tasks.discard(task)
       └─ if task.exception(): logger.error(...)

Agent.flush()  ← graceful shutdown
  └─ await asyncio.gather(*self._bg_tasks, return_exceptions=True)
```

### 测试统计: 45 项全部通过 ✅

- 单元测试 (`tests/unit/test_phase_s.py`): 40 项
  - S1 错题次数: 2 项
  - S2 后台任务跟踪: 3 项
  - S3 asyncio.to_thread: 3 项
  - S4 verdict 标准化: 3 项
  - S5 FeedbackStore async: 3 项
  - S6 安全模板: 2 项
  - S7 flush: 2 项
  - S8 UTC 时区: 2 项
  - S9 JSON 异常保护: 4 项
  ...
  - S14 批量 decay: 2 项
  - S15-S16 SQL 过滤: 2 项
  - S20 safe_parse_json: 4 项
- 端到端测试 (`tests/e2e/test_phase_s_e2e.py`): 5 项
  - Agent 后台任务完整生命周期
  - Memory 模块数据损坏恢复
